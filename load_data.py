...
def load_data(batch_size):
    ...
    #val_iter = torch.utils.data.DataLoader(Dataset(is_train=True),batch_size,shuffle=True)
...
def read_data(is_train=True):
    ...
    for img_name, target in csv_data.iterrows():
        images.append(torchvision.io.read_image(
            os.path.join(data_dir,'sysu_train' if is_train else 'sysu_val', 'images',f'{img_name}')))
...
batch_size = 32
tarin_iter = load_data(batch_size)